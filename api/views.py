...
from .serializers import PatientInformationSerializer,LaboratoryMeasurementSerializer, PredictionResultSerializer
...
def getPatientInformation(request,pk):
    notes=PatientInformation.objects.get(id=pk)
    serializer=PatientInformationSerializer(notes,many=False)
    return Response(serializer.data)
...
def getLaboratoryInformation(request,pk):
    notes=LaboratoryMeasurement.objects.get(patientId=pk)
